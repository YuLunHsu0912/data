...
def color_splash(image, mask):
    ...
    now=np.copy(gray)
    for a in range(now.shape[0]):
      for b in range(now.shape[1]):
        now[a][b]=0
...
if __name__ == '__main__':
    ...
    if args.weights.lower() == "coco":
        ...
        weights_path = os.path.join(MODEL_DIR, "mask_rcnn_mango.h5")
    ...
    model_path = os.path.join(MODEL_DIR, "mask_rcnn_mango.h5")
